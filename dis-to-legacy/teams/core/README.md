# core — moving services out of the legacy cluster
...
`altinn-notifications/httproute.yaml`. Near the bottom there is a commented-out block —
uncomment it and fill in the namespace your service runs in:

```yaml
- name: altinn-notifications
...
`envs/<env>/core/kustomization.yaml`. That way you change one environment at
